...
### 백엔드 번들 사본 (배포 필수)
Railway 백엔드는 루트가 `backend/`라 컨테이너에 `frontend/`가 없다. 그래서 백엔드
(`legionella_lib.load_facilities`·`/surveillance/example`)는 데이터를 두 곳에서 순서대로 찾는다:
`frontend/public/data`(로컬) → `backend/app/legionella_data`(배포 번들). **데이터를 재생성하면 번들 사본도 갱신**해야 배포에 반영된다:
```bash
cp frontend/public/data/facilities_bath.geojson frontend/public/data/facilities_hospital.geojson backend/app/legionella_data/
cp frontend/public/data/synthetic_surveys/case_0*.txt backend/app/legionella_data/synthetic_surveys/
```
