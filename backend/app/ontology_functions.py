...
def _what_if_outbreak_national(inputs: dict) -> dict:
    ...
    if observed_only:
        ...
        comparison = {
            ...
            "note": "A(실측값+중력장 모형, 기본)는 관측 OD 경로를 우선하되 나머지를 중력장(인구·거리²) 이동으로 채워 모든 지역을 연결합니다. B(실측값만)는 측정된 OD 경로에만 유입 압력을 집중시켜 관측 구간을 강하게 밝히지만, 실측이 없는 지역은 고립되어 전혀 도달하지 못합니다. 그래서 R0가 낮을 때는 B가 관측 구간에 집중돼 더 넓어 보일 수 있으나, 미관측 지역까지 현실적으로 확산하는 A가 기본값입니다. 아래 통계·애니메이션은 A/B를 전환해 볼 수 있습니다.",
        }
    else:
